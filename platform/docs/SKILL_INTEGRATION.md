...
Verified on **September 18, 2026**. Objective: build a skill that discovers Algoria services, pays through x402 using a wallet on the user's own computer, and retrieves the result.
...
Algoria offers its own services. The current service generates images from text. fal is Algoria's server-side provider; users do not connect to fal or supply a fal key.
...
8. Present the image and payment receipt to the user; resume the same job after interruptions.
...
Full JSON reference copies based on the live API responses fetched on the same date accompany this document. Discovery tags in the catalog and service copies have been normalized to English to match the repository metadata; these are not byte-for-byte snapshots of the current deployment. Endpoint and payment fields are unchanged.
...
- `GET /v1/jobs/{job_id}`: job status, payment receipt, and current image URL. Requires the job's recovery token; does not initiate a new payment or generation.
...
This example shows the empty-list format. When services match, `resources` contains complete service documents. There is currently one service: `image.generate`, version `"1"`.
...
The current service price is `100000` atomic units, or **0.01 test USDC**. One USDC equals `10000000` atomic units. Calculate amounts using strings and `BigInt`, not floating point. The price, the free mock TRY top-up amount, and fal's actual cost to Algoria are separate quantities.
...
The current service generates one square 1K image. The server uses `google/nano-banana-2-lite`; provider/model selection is not a public input.

```json
{"prompt":"A small red sailboat on a calm turquoise sea, watercolor illustration."}
```

The only input field is `prompt`; additional fields are rejected. Empty or whitespace-only values are not accepted. The upper limit is 4000 according to JavaScript string length; validation occurs before trimming, and leading/trailing whitespace is removed afterward. The HTTP JSON body may contain at most 32768 bytes. Check both limits because multibyte characters and JSON escaping affect body size.
...
For the same job, the service, body, idempotency key, and recovery token remain fixed. `mode` and `wait_ms` may change. Different input under the same ID for the same recognized service returns `409 request-conflict`; an unknown service path returns `404 service-not-found`. Invalid-token access is hidden behind `404`.

## 8. Initial 402 → local signature → execution

The HTTP examples below are templates; do not send `<...>` placeholders literally. Do not generate a new UUID on each retry.
...
## 9. Sync, async, and image results
...
`width` and `height` are optional. The service targets PNG; the output schema accepts PNG/JPEG/WebP, and the actual `content_type` is authoritative. Not every receipt includes `payment.amount`; obtain the amount from the job's saved offer. Account for a receipt only once, using `job_id` and transaction hash.
...
Images are stored in private Supabase Storage; the supplied signed URL is valid for 3600 seconds. Anyone with the URL can read the image. When downloading it, **do not send the Algoria recovery token or anchor JWT**. If the URL expires, obtain a new one through an authenticated job GET; do not regenerate or pay again. If the user wants a persistent local file, download the image on the client; the service does not guarantee indefinite retention.
...
- **`succeeded`**: Present the image, save the receipt once, and mark the local job complete.
...
- `400 invalid-json`, `invalid-input`: the body or prompt does not satisfy the contract.
...
- `404 service-not-found`: recheck the service in the catalog.
...
The current backend has an **operator setting** added during implementation to control fal costs: 10 paid generation attempts in total and 2 active jobs at a time. This is not a per-user or daily product quota, nor a membership/plan feature requested by the user.
...
An operator can change the setting through `public.demo_settings`. There is no public endpoint to read or modify capacity. Do not hardcode product rules such as “you have 10 images” or “7 remaining” into the skill. The skill manages its own local budget and treats an API 429 as server capacity. The limit was not changed while preparing this document.
...
Expose operation results, the public wallet address, balance, job ID, safe errors, and images to the LLM layer. Generate and send payment headers inside the trusted helper; there is no need to pass a header through model output.
...
1. **Discovery:** query finds the service and uses its actual resource/input_schema/accepts. A second service fixture retains its distinct payTo.
2. **Input:** empty, excessively long, or extra-field input is rejected locally; a valid prompt is sent as correct JSON.
...
7. **Sync success:** 200 + succeeded + image + successful receipt are saved once.
...
10. **HTTP loss/restart:** resume from the local record after a paid POST response is lost or the application closes; preserve the ID/token.
...
15. **Output:** renew an expired signed URL through job GET. Do not attach Algoria/anchor credentials to the image request.
...
The existing backend validation passed 43 automated tests and three live scenarios (sync, async, and sync→202 fallback). This evidence concerns the backend; it does not mean the unwritten skill has been tested.
...
The next developer's task is to connect discovery, local wallet/top-up, controlled x402 payment, persistent job recovery, and image presentation to the skill environment using this HTTP contract. The API endpoints are operational. Skill packaging, command names, and any future MCP tools are outside this document's scope and will be chosen during that implementation.
